...
export function formatPrescription(item: PrescribedItem): string {
  ...
  } else if (item.sets != null && item.repsRangeMin != null && item.repsRangeMax != null) {
    prescription = `${item.sets}×${item.repsRangeMin}–${item.repsRangeMax}`;
  } else if (item.sets != null && item.durationSec != null) {
    prescription = `${item.sets}×${item.durationSec} s`;
  } else if (item.durationSec != null) {
    prescription = item.durationSec >= 60 ? `${item.durationSec / 60} min` : `${item.durationSec} s`;
  }
